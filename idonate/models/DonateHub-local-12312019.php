...
class DonateHub extends Database{
	...
	*/
	public function get( $params ){
		...
		$return_data = array();
		if($status == 'All'){
			$query = "SELECT * FROM `".$this->transactions."` WHERE `bbec_status` IN ('Pending','Done') ORDER BY `created_datetime`" . $order;
		}else{
...
/* 			$message = $e->getMessage();
	...
	*/ 
	...
	/**
	* Expunge records from queue
	* @param $num_days_back - Number of days back to begin removal from; Default 10
	*
	*/
	public function expunge($num_days_back = 10){

		$stmt = $this->conn->prepare("DELETE FROM `".$this->transactions."` WHERE `created_datetime` <= DATE(NOW() - INTERVAL :num_days_back DAY) AND `bbec_status` = 'Done'");
		$stmt->bindParam(':num_days_back', $num_days_back, PDO::PARAM_INT);
		$stmt->execute();
